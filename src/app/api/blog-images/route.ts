 ...
 * Uploads an image to the blog-images bucket. Admin only (Supabase or master).
 */
export async function POST(request: NextRequest) {
  ...
  const { data: urlData } = auth.supabase.storage.from('blog-images').getPublicUrl(data.path);
